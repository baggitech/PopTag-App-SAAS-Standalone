...
class PayLoadPix extends Model
{
  ...
  */
  ...
  {
  	// CRIA O PAYLOAD
  	$payload = $this->getValue(self::ID_PAYLOAD_FORMAT_INDICATOR, '01'); // 00
  	$payload .= $this->getMerchantAccountInformation(); // 26
  	$payload .= $this->getValue(self::ID_MERCHANT_CATEGORY_CODE, '0000'); // 52
  	$payload .= $this->getValue(self::ID_TRANSACTION_CURRENCY, '986'); // 53
    $payload .= $this->getValue(self::ID_TRANSACTION_AMOUNT, $this->amount); // 54
    $payload .= $this->getValue(self::ID_COUNTRY_CODE, 'BR'); // 58 	
  	$payload .= $this->getValue(self::ID_MERCHANT_NAME, $this->merchantName); // 59
  	$payload .= $this->getValue(self::ID_MERCHANT_CITY, $this->merchantCity); // 60
